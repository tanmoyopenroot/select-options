/**
 * Action types for redux actions
 * @module actions/action-types
 */

export const FETCH_DATA = 'FETCH_DATA';
export const FETCHED_USERS_DATA = 'FETCHED_USERS_DATA';
export const FETCHED_ROLES_DATA = 'FETCHED_ROLES_DATA';
export const FETCHED_PROJECT_DATA = 'FETCHED_PROJECT_DATA';

export const FETCHING_ERROR_USERS_DATA = 'FETCHING_ERROR_USERS_DATA';
export const FETCHING_ERROR_ROLES_DATA = 'FETCHING_ERROR_ROLES_DATA';
export const FETCHING_ERROR_PROJECTS_DATA = 'FETCHING_ERROR_PROJECTS_DATA';


export const SELECTED_DATA = 'SELECTED_DATA';

export const SET_SELECTED_USER = 'SET_SELECTED_USER';
export const SET_SELECTED_ROLE = 'SET_SELECTED_ROLE';
export const SET_SELECTED_PROJECT = 'SET_SELECTED_PROJECT';
